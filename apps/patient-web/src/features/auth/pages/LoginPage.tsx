...
import { loginPatientSchema, type LoginPatientInput } from '@clinicos/validation';
...
export default function LoginPage() {
  ...
  const login = useLoginPatientMutation();
  const {
    register,
    handleSubmit,
    formState: { errors, isSubmitting },
    setError,
  } = useForm<LoginPatientInput>({ resolver: zodResolver(loginPatientSchema) });
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <div className="w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-sm text-text-secondary">Access your healthcare account</p>
        </div>
